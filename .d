...
def register():
    if request.method == "POST":
        ...
        # Only check for email address collisons; don't use username.
        rows = db.execute("SELECT * FROM users WHERE username = ?", request.form.get("username"))
        if len(rows) == 1:
            flash("Username taken!")
            return render_template("register.html")
        # Do cheap operations first; check password match before querying DB for email collision.
        # This should also be done in JS; verified here.
        if request.form.get("password") != request.form.get("confirmation"):
            flash("Passwords don't match!")
            return render_template("register.html")
        
        # Ideally, create a new table in the database called registration; add columns for unique ID (primary key, auto-increment), email, verification code, and timestamp.
        # Insert record here so it can be checked later; don't rely on session storage, as it can be manipulated.
        # When checking response, verify in registration table, compare token value, remove if found (pattern: nonce (number used once)).
        # Also, ensure timestamp is within last n minutes, where n = 5? 30? Also remove any records from this table that have an expiration older than that value to keep it clean.
        ...
        # Should have more instructional copy telling the user what to do with this code, provide a link maybe?
    ...
    if request.method == "POST":
        ...
        if request.form.get("code") == session['verification_data']['code']:
            db.execute(
                ...
                session['verification_data']['username'],
                generate_password_hash(session['verification_data']['password']), 
                session['verification_data']['firstname'], 
                session['verification_data']['lastname'], 
...
def login():
    ...
    # Forget any user_id
    # KEVIN: I wouldn't do this. I'd redirect them to the homepage if they're already logged in. People bookmark different pages, they might bookmark the login page, don't force-log them out.
    session.clear()
...
def product(product):
  if request.method == "POST":
        redirect("/cart")
        # Don't trust user input! Does this product exist? Is the quantity a positive integer in a reasonable range?
        product = request.form.get("product")
        quantity = request.form.get("quantity")
        modifiers = request.form.get("modifiers")
        price = request.form.get("price")
        # Verify user ID from session (does user ID exist)?
        # Even better, store a randomly generated session token in the DB + session, ensure both match; reset session token when user logs out.
        db.execute("INSERT INTO cart (user, product, quantity, modifiers,price) VALUES (?,?,?,?,?)", session["user_id"], product, quantity, modifiers, price)
        return redirect("/cart")
  else:
        # Never do select *! Always ask for what you need.
...
@login_required
def cart():
    # For any routes that require authentication, ensure user is properly authenticated before proceeding.
    ...
    if request.method == "POST":
        ...
        def makeLineItem(item):
            return {
                ...
                # Tax? See if you can get Stripe to do this for you.
            }
        # If you use the map, you don't have to calculate the total, Stripe will do it for you.
        for index in cart:
            price_total += (index['price'] * index['quantity'])
        stripe_payment_processing = stripe.checkout.Session.create(
            line_items = map(makeLineItem, cart),
...
def order():
    ...
    if check_password_hash(request.args.get('key', default=""),session['stripeURL']):
        ...
        for product in cart:
            db.execute("INSERT INTO orders (orderid, user, product,quantity,modifiers,completed,total,date) VALUES (?,?,?,?,?,?,?,?)", id,session["user_id"], product['product'], product['quantity'],product['modifiers'],0,product['price'], datetime.now())
    else:
        success = False
    if success:
    ...
    # Make this contingent upon time of day; reference open hours.
...
def orders():
    if request.method == "POST":
        ...
        for product in order_data:
            db.execute("INSERT INTO cart (user, product, quantity, modifiers, price) VALUES (?, ?, ?, ?, ?)",session["user_id"], product["product"], product["quantity"], product["modifiers"], product["total"])
...
def admin_login():
    if request.method == "POST":
        try:
            # Tie this to user role. Create a table for user roles, and if a user doesn't have an entry in that table, assume they are a customer only; else have roles set for certain users (e.g., administrator).
            passkey = "1981"
...
-- To get all items in an order:
SELECT order_items.id, order_items.order_id, order_items.product
FROM orders INNER JOIN order_items ON orders.id = order_items.order_id
WHERE orders.id = ?; -- pass in order ID